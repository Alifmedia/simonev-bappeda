...
class AdminMatrixController extends Controller
{
    ...
    public function matrixRfk(Request $req)
    {
      ...
      $filter['bagian']   = Bagian::all();
       ...
       $data = Renssasar::with('indikatorrenssasar.renssasarprog.indikatorrenssasarprog.renssasarprogkeg')->get();
      ...
      return view('view_admin_matrix.admin_matrix_rfk')->with('filter', $filter)
                                                      ->with('data', $data);
    }
...
//carilah semua KEGIATAN/GAMPONG YANG TELAH DIMASUKKAN KEDALAM RENSSASARPROGKEG YANG MANA UMUM_ID OPD nya adalah sama dengan session
  ...
  $data = Kabupaten::whereHas('kecamatan.kemukiman.renssasarprog.indikatorrenssasar.renssasar.indikatortujuan.tujuan.renstra.sektoral'                              )->get();
...
return view('view_admin_matrix.admin_matrix_renja')->with('filter', $filter)
