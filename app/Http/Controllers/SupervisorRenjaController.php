...
class SupervisorRenjaController extends Controller
{
...
public function deleteKegiatan(Request $req)
    ...
    // }
      ...
      return view('view_supervisor_renja.renja_setup')->with('filter', $filter)
                                              // ->with('bagian', $bagian)
                                              // ->with('hal', $hal)
                                              // ->with('unsur', $unsur)
                                               ->with('data', $data);
...
//carilah semua KEGIATAN/GAMPONG YANG TELAH DIMASUKKAN KEDALAM RENSSASARPROGKEG YANG MANA UMUM_ID OPD nya adalah sama dengan session
      $data = Indikatortujuan::with('renssasar.indikatorrenssasar.renssasarprog.indikatorrenssasarprog.renssasarprogkeg')->whereHas('tujuan.renstra.sektoral.umumsektoral',
                                            function($query)
                                             {
    ...
    // }
      ...
      return view('view_supervisor_renja.renja_setujuopd')->with('filter', $filter)
...
public function createSubkegiatan($renssasarprogkeg_id, $kegiatan_id, $kegiatan_kode, $kegiatan_nama)
    {
     ...
     return view('view_supervisor_renja.renja_add')->with('renssasarprogkeg_id', $renssasarprogkeg_id)
    ...
    //                 .str_pad(((int)$jml_konsultasi_hari_ini)+1,4,'0',STR_PAD_LEFT);
      ...
      $data->renssasarprogkegsubkeg_targetkinerja = $req->target;
      ...
      return redirect()->route('supervisor.renja.setujuopd')
...
//carilah semua KEGIATAN/GAMPONG YANG TELAH DIMASUKKAN KEDALAM RENSSASARPROGKEG YANG MANA UMUM_ID OPD nya adalah sama dengan session
  ...
  $data = Renssasarprog::whereHas('indikatorrenssasar.renssasar.indikatortujuan.tujuan.renstra.sektoral.umumsektoral',
                                            function($query)
                                             {
                                              $query->where('umum_id', session('umum_atau_administrator_id'));
                                             }
    ...
    // }
      ...
      return view('view_supervisor_renja.renja_tahunan')->with('filter', $filter)
    ...
    // }
      ...
      return view('view_supervisor_renja.renja_rekapitulasi')->with('filter', $filter)
                                              // ->with('bagian', $bagian)
                                              // ->with('hal', $hal)
                                              // ->with('unsur', $unsur)
                                               ->with('data', $data);
