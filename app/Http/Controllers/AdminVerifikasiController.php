...
class AdminVerifikasiController extends Controller
{
    ...
    public function tampilKecamatan()
    {
      ...
      $data = Kecamatan::where('status_program','aktif')->get();
                                                          
                          

          
      return view('view_admin_verifikasi.admin_kec')->with('data', $data);
                                                             //  ->with('progress', $value->progress);
    }


    public function tampilTema($id)
    {
      ...
      $data = TemaSoal::with(['soal.jawabanSoalKecamatan'=>function($q)use($id)
                                                           {
      ...
      //   {
      //     $bobot = $soal->jawabanSoalKecamatan->isNotEmpty() ? 
      //              $soal->jawabanSoalKecamatan[0]->jawaban->bobot:0;
      ...
      return view('view_admin_verifikasi.admin_kecamatan')->with('data', $data);
                                                              // ->with('progress', $value->progress);
    ...
    public function survey($id)
    {
      ...
      // $data = Kecamatan::where('id',$id)->with(['jawabanSoalKecamatan'=>function($q)use($id)
      //                                                                   {
      //                                                                     $q->where('kecamatan_id',$id);
      //                                                                   }, 
      //                                           'jawabanSoalKecamatan.Soal.jawaban_soal.jawaban'
      //                                         ])->firstOrFail();


      // $data = Soal::with(['jawabanSoalKecamatan'=>function($q)use($id)
      //                                             {
      //                                               $q->where('kecamatan_id',$id);
      //                                             }, 
      //                     'JawabanSoal.Jawaban'
      ...
      // $data = Soal::All();
        ...
        $data = TemaSoal::where('id', $id)->with(['soal.jawaban', 
                                                'soal.jawabanSoalKecamatan'=>function($q)use($id)
                                                                    {
                                                                      $q->where('kecamatan_id',$id);
                                                                    }
                                                ])->firstOrFail();
      ...
      //dd($id);
      return view('view_admin_verifikasi.admin_kecamatan_survey')->with('data', $data);
    }
     ...
     public function saveSurvey(Request $req, $id)
    {
        ...
        return redirect()->route('admin.kecamatan')->with('success','Data berhasil disimpan');
...
//MENGAMBIL DATA BOBOT DARI 
    public function hasilSurvey()
    {
      ...
      // $data = TemaSoal::all();
      return view('view_admin_verifikasi.admin_kecamatan_hasil')->with('data', $data);
    }
