...
class KaryawanRenjaController extends Controller
{
...
//carilah semua KEGIATAN/GAMPONG YANG TELAH DIMASUKKAN KEDALAM RENSSASARPROGKEG YANG MANA UMUM_ID OPD nya adalah sama dengan session
  ...
  $data = Renssasarprog::whereHas('indikatorrenssasar.renssasar.indikatortujuan.tujuan.renstra.sektoral.umumsektoral',
    ...
    // }
      ...
      return view('view_karyawan_renja.renja_tahunan')->with('filter', $filter)
 ...
 public function rekapitulasi(Request $req)
    ...
    // }
      ...
      return view('view_karyawan_renja.renja_rekapitulasi')->with('filter', $filter)
                                              // ->with('bagian', $bagian)
                                              // ->with('hal', $hal)
                                              // ->with('unsur', $unsur)
                                               ->with('data', $data);
...
public function deleteKegiatan(Request $req)
     ...
     public function setup(Request $req)
    {
      ...
      $data = Dusun::with(['gampong.kemukiman.kecamatan.kabupaten']);
        ...
        elseif ($req->kemukiman) 
     {
        $data = $data->whereHas('gampong', function($query)use($req)
                                                                {
                                                                    $query->where('kemukiman_id',$req->kecamatan);
    ...
    // }
      ...
      return view('view_karyawan_renja.renja_setup')->with('filter', $filter)
                                              // ->with('bagian', $bagian)
                                              // ->with('hal', $hal)
                                              // ->with('unsur', $unsur)
                                               ->with('data', $data);
...
//carilah semua KEGIATAN/GAMPONG YANG TELAH DIMASUKKAN KEDALAM RENSSASARPROGKEG YANG MANA UMUM_ID OPD nya adalah sama dengan session
    ...
    // }
      ...
      return view('view_karyawan_renja.renja_setujuopd')->with('filter', $filter)
